...
import { VoteResult } from '@/types/stream';
...
const useVoteSubscription = (props: useVoteSubscriptionProps) => {
  ...
  const { stompClient, setVote, setIsVoteShow, setIsSelected, setIsVoteFinished, setVoteResult } =
    useSocketStore();

  useEffect(() => {
    if (streamId && streamId !== '-1' && stompClient) {
      const getCurrentVoteSubscribe = stompClient.subscribe(
        END_POINTS.SUBSCRIBE.VOTE_CURRENT(stompClient.ws._transport.url.split('/')[5]),
        (message) => {
      ...
      stompClient.send(END_POINTS.PUBLISH.VOTE_LIST_REQUEST(streamId), {}, JSON.stringify({}));
      ...
      if (getCurrentVoteSubscribe) getCurrentVoteSubscribeRef.current = getCurrentVoteSubscribe;
      if (voteCloseAndResultSubscribe)
        voteCloseAndResultSubscribeRef.current = voteCloseAndResultSubscribe;
      if (createVoteSubscribe) createVoteSubscribeRef.current = createVoteSubscribe;
    }
